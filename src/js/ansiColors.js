 ...
 * @param {[number, number, number]} ref [Red, Green, Blue]
 * @returns {string}
 */
...
function notCategory(category) {
    return e => (category === null || e.category !== category) && category !== 'all';
}
...
export default function (input) {
    ...
    tokenize(input.join(''), options, (token, data) => {
        ...
        if (options.stream) {
            stickyStack = updateStickyStack(stickyStack, token, data);
        }
    });
